...
interface ContributorListProps {
  ...
  onEdit: (contributor: PlayContributor) => void;
  onDelete: (contributorNo: number) => void;
  onView: (contributor: PlayContributor) => void;
  onAdd: () => void;
  deleting: number[];
...
export default function ContributorList({ contributors, onEdit, onDelete, onView, onAdd, deleting, showSearchForm, onToggleSearch }: ContributorListProps) {
  const [peopleNoFilter, setPeopleNoFilter] = useState<string>("");
  const [typeFilter, setTypeFilter] = useState<string>("");
  const [playNoFilter, setPlayNoFilter] = useState<string>("");
  const [appliedPeopleNo, setAppliedPeopleNo] = useState<string>("");
  const [appliedType, setAppliedType] = useState<string>("");
  const [appliedPlayNo, setAppliedPlayNo] = useState<string>("");

  const filtered = useMemo(() => {
    return contributors.filter(c => {
      if (appliedPeopleNo && String(c.people_no ?? '').indexOf(appliedPeopleNo) === -1) return false;
      if (appliedType && !(c.play_contributor_type || '').toLowerCase().includes(appliedType.toLowerCase())) return false;
      if (appliedPlayNo && String(c.play_no ?? '').indexOf(appliedPlayNo) === -1) return false;
      return true;
    });
  }, [contributors, appliedPeopleNo, appliedType, appliedPlayNo]);

  const handleSearch = () => {
    setAppliedPeopleNo(peopleNoFilter);
      ...
      {showSearchForm && (
        <div className="mb-4">
          <ContributorSearchForm
            defaultFilters={{ peopleNo: appliedPeopleNo, type: appliedType, playNo: appliedPlayNo }}
            onSearch={(f)=>{
              setPeopleNoFilter(f.peopleNo);
              setTypeFilter(f.type);
              setPlayNoFilter(f.playNo);
              setAppliedPeopleNo(f.peopleNo);
              setAppliedType(f.type);
        ...
        <table className="w-full table-auto">
          <thead>
            <tr className="bg-gray-2 text-left dark:bg-meta-4">
              <th className="px-4 py-4 font-medium text-black dark:text-white">PC No</th>
              <th className="px-4 py-4 font-medium text-black dark:text-white">People No</th>
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-4 py-8 text-center text-gray-400">No contributors found</td>
              </tr>
            ) : null}
            {filtered.map((contributor) => {
              const isDeleting = deleting.includes(contributor.pc_no);
              
              return (
                <tr key={contributor.pc_no} className="border-b border-stroke dark:border-strokedark">
                  <td className="px-4 py-4">{contributor.pc_no}</td>
                  <td className="px-4 py-4">{contributor.people_no ?? '-'}</td>
                  <td className="px-4 py-4">{contributor.play_contributor_type}</td>
                  <td className="px-4 py-4">{contributor.play_no ?? '-'}</td>
                  <td className="px-4 py-4">
                    <div className="flex items-center space-x-3.5">
                      ...
                      <button
                        ...
                        onClick={() => !isDeleting && onDelete(contributor.pc_no)}
                        title="Delete"
                        disabled={isDeleting}
                      >
                        {isDeleting ? (
