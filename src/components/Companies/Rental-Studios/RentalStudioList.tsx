...
interface RentalStudioListProps {
  rentalStudios: (RentalStudio & { company_name?: string })[];
  onEdit: (rentalStudio: RentalStudio) => void;
  onDelete: (rentalStudio: number) => void;
  onView: (rentalStudio: RentalStudio) => void;
  onComments: (rentalStudio: RentalStudio) => void;
  deleting: number[];
}

export default function RentalStudioList({ 
  rentalStudios, 
  onEdit, 
  onDelete, 
  onView, 
  onComments, 
  deleting 
}: RentalStudioListProps) {
  if (rentalStudios.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        <p className="text-gray-500 dark:text-gray-400">No rental studios found</p>
      </div>
    );
  }
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full table-auto">
        ...
        <tbody>
          {rentalStudios.map((rentalStudio) => {
            const isDeleting = deleting.includes(rentalStudio.studio_no);

            return (
              <tr key={rentalStudio.studio_no} className="border-b border-stroke dark:border-strokedark">
                <td className="py-5 px-4">
                  <p className="text-black dark:text-white">{rentalStudio.studio_no}</p>
                </td>
                <td className="py-5 px-4">
                  <p className="text-black dark:text-white font-medium">
                    {rentalStudio.name || `Company #${rentalStudio.company_no}`}
                  </p>
                </td>
                <td className="py-5 px-4">
                  <p className="text-sm text-black dark:text-white">
                    {rentalStudio.address_no || "-"}
                  </p>
                </td>
                <td className="py-5 px-4">
                  <p className="text-sm text-black dark:text-white">
                    {rentalStudio.num_studios || "-"}
                  </p>
                </td>
                <td className="py-5 px-4">
                  <p className="text-sm text-black dark:text-white">
                ...
                <td className="py-5 px-4">
                  <div className="flex items-center space-x-3.5">
                    ...
                    <button
                      ...
                      onClick={() => onComments(rentalStudio)}
                      title="Comments"
                      disabled={isDeleting}
                    >
                      <svg
                    ...
                    <button
                      ...
                      onClick={() => !isDeleting && onDelete(rentalStudio.studio_no)}
                      title="Delete"
                      disabled={isDeleting}
                    >
                      {isDeleting ? (
