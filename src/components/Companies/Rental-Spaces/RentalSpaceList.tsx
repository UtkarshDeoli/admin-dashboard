...
interface RentalSpaceListProps {
  rentalSpaces: (RentalSpace & { company_name?: string })[];
  onEdit: (agency: RentalSpace) => void;
  onDelete: (agencyNo: number) => void;
  onView: (agency: RentalSpace) => void;
  onComments: (agency: RentalSpace) => void;
  deleting: number[];
}

export default function RentalSpaceList({ 
  rentalSpaces, 
  onEdit, 
  onDelete, 
  onView, 
  onComments, 
  deleting 
}: RentalSpaceListProps) {
  if (rentalSpaces.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        <p className="text-gray-500 dark:text-gray-400">No castings found</p>
      </div>
    );
  }
      ...
      <table className="w-full table-auto">
        ...
        <tbody>
          {rentalSpaces.map((rentalSpace) => {
            const isDeleting = deleting.includes(rentalSpace.space_no);

            return (
              <tr key={rentalSpace.space_no} className="border-b border-stroke dark:border-strokedark">
                <td className="py-5 px-4">
                  <p className="text-black dark:text-white">{rentalSpace.space_no}</p>
                </td>
                <td className="py-5 px-4">
                  <p className="text-black dark:text-white font-medium">
                    {rentalSpace.name || `Company #${rentalSpace.company_no}`}
                  </p>
                </td>
                <td className="py-5 px-4">
                  <p className="text-sm text-black dark:text-white">
                    {rentalSpace.address_no || "-"}
                  </p>
                </td>
                <td className="py-5 px-4">
                  <p className="text-sm text-black dark:text-white">
                    {rentalSpace.dimensions || "-"}
                  </p>
                </td>
                <td className="py-5 px-4">
                  <p className="text-sm text-black dark:text-white">
                ...
                <td className="py-5 px-4">
                  <div className="flex items-center space-x-3.5">
                    ...
                    <button
                      ...
                      onClick={() => onComments(rentalSpace)}
                      title="Comments"
                      disabled={isDeleting}
                    >
                      <svg
                    ...
                    <button
                      ...
                      onClick={() => !isDeleting && onDelete(rentalSpace.space_no)}
                      title="Delete"
                      disabled={isDeleting}
                    >
                      {isDeleting ? (
