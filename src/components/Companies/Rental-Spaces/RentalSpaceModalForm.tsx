...
import { RentalSpace, Company, Address } from "@/types/company";

interface RentalSpaceModalFormProps {
...
export default function RentalSpaceModalForm({
    rentalSpace,
    onSave,
    onCancel,
    saving
}: RentalSpaceModalFormProps) {
    const [formData, setFormData] = useState({
        space_no: rentalSpace?.space_no || 0,
        company_no: rentalSpace?.company_no || 0,
        address_no: rentalSpace?.address_no || 0,
        name: rentalSpace?.name || '',
        dimensions: rentalSpace?.dimensions || '',
        seats: rentalSpace?.seats || 0,
        space_type: rentalSpace?.space_type || '',
        archived: rentalSpace?.archived || false,
    });
    ...
    useEffect(() => {
        ...
        if (rentalSpace) {
            setFormData({
                space_no: rentalSpace.space_no,
                company_no: rentalSpace.company_no,
                address_no: rentalSpace.address_no,
                name: rentalSpace.name,
                dimensions: rentalSpace.dimensions,
                seats: rentalSpace.seats,
                space_type: rentalSpace.space_type,
                archived: rentalSpace.archived,
            });

            // Load the company name for editing
            if (rentalSpace.company_no) {
                loadCompanyName(rentalSpace.company_no);
            }
        }
    }, [rentalSpace]);
                    ...
                    >
                        {saving ? (
                            <div className="flex items-center gap-2">
                                ...
                                {rentalSpace ? "Updating..." : "Creating..."}
                            </div>
                        ) : (
                            rentalSpace ? "Update Rental Space" : "Create Rental Space"
                        )}
                    </button>
                    <button
                        type="button"
