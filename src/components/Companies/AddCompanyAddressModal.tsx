...
interface AddCompanyAddressModalProps {
  isOpen: boolean;
  onClose: () => void;
  addresses: Address[];
  onAdd: (addressId: number, locaction: string) => Promise<void>;
  saving: boolean;
}

export default function AddCompanyAddressModal({
  isOpen,
  onClose,
  addresses,
  onAdd,
  saving
}: AddCompanyAddressModalProps) {
  const [selectedAddressId, setSelectedAddressId] = useState<number | null>(null);
  const [locaction, setLocaction] = useState("");
  const [searchTerm, setSearchTerm] = useState("");

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (selectedAddressId) {
      await onAdd(selectedAddressId, locaction);
        ...
        <div className="relative flex flex-col w-full bg-white border-0 rounded-lg shadow-lg outline-none focus:outline-none dark:bg-boxdark">
          ...
          <div className="flex items-start justify-between p-5 border-b border-solid rounded-t border-stroke dark:border-strokedark">
            <h3 className="text-xl font-semibold text-black dark:text-white">
              Add Existing Address to Company
            </h3>
            <button
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  value={locaction}
                  onChange={(e) => setLocaction(e.target.value)}
                  className="w-full rounded border-[1.5px] border-stroke bg-transparent px-5 py-3 text-black outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                />
              </div>
            </form>
