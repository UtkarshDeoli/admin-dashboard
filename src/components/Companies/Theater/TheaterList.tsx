  ...
  if (theaters.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        <p className="text-gray-500 dark:text-gray-400">No theaters found</p>
      </div>
    );
  }
      ...
      <table className="w-full table-auto">
        ...
        <tbody>
          {theaters.map((theater) => {
            const isDeleting = deleting.includes(theater.theater_no);

            return (
              <tr key={theater.theater_no} className="border-b border-stroke dark:border-strokedark">
                <td className="py-5 px-4">
                  <p className="text-black dark:text-white">{theater.theater_no}</p>
                </td>
                <td className="py-5 px-4">
                  <p className="text-black dark:text-white font-medium">
                    {theater.company_name || `Company #${theater.company_no}`}
                  </p>
                </td>
                <td className="py-5 px-4">
                ...
                <td className="py-5 px-4">
                  <td className="py-5 px-4">
                    <span className={`inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium ${theater.production_compnay
                      ? "bg-success text-success"
                      : "bg-warning text-warning"
                      }`}>
                      {theater.production_compnay ? "Yes" : "No"}
                    </span>
                  </td>
                </td>
                <td className="py-5 px-4">
                  <span className={`inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium ${theater.summer
                    ? "bg-success text-success"
                    : "bg-warning text-warning"
                    }`}>
                    {theater.summer ? "Yes" : "No"}
                  </span>
                </td>
                <td className="py-5 px-4">
                  <p className="text-sm text-black dark:text-white">
                    {theater.literary_submission_preference ? (
                      theater.literary_submission_preference.length > 50
                        ? `${theater.literary_submission_preference.substring(0, 50)}...`
                        : theater.literary_submission_preference
                    ) : "-"}
                  </p>
                </td>
                <td className="py-5 px-4">
                  <span className={`inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium ${theater.submission_preference
                    ? "bg-success text-success"
                    : "bg-warning text-warning"
                    }`}>
                    {theater.submission_preference ? "Yes" : "No"}
                  </span>
                </td>
                <td className="py-5 px-4">
                  <div className="flex items-center space-x-3.5">
                    ...
                    <button
                      ...
                      onClick={() => !isDeleting && onDelete(theater.theater_no)}
                      title="Delete"
                      disabled={isDeleting}
                    >
                      {isDeleting ? (
