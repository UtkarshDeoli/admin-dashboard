...
export default function CastingManager() {
  ...
  const filterSchools = useCallback(() => {
    ...
    if (searchFilters.online) {
      filtered = filtered.filter(school =>
        school.online === (searchFilters.online === 'true')
      );
    }

    // Filter by In-person status
    if (searchFilters.in_person) {
      filtered = filtered.filter(school =>
        school.in_person === (searchFilters.in_person === 'true')
      );
    }

    setFilteredSchool(filtered);
  }, [school, searchFilters]);
  ...
  const handleDelete = async (schoolNo: number) => {
    ...
    if (confirmed) {
      try {
        ...
        const response = await fetch(`/api/school/${schoolNo}`, {
          method: 'DELETE',
        });
  ...
  const handleSave = async (schoolData: Omit<School, 'school_no'>) => {
    try {
      ...
      if (editingSchool) {
        ...
        const response = await fetch(`/api/school/${editingSchool.school_no}`, {
          method: 'PUT',
          headers: {
  ...
  const handleSearch = async (filters: { companyName: string; online: string; in_person: string }) => {
    try {
      setSearching(true);
      setSearchFilters(filters);

      if (filters.companyName.trim() === '' && filters.online.trim() === '' && filters.in_person.trim() === '') {
        await loadSchools();
      }
    } catch (err) {
  ...
  useEffect(() => {
    filterSchools();
  }, [school, searchFilters, filterSchools]);
