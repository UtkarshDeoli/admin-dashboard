...
interface CastingViewModalProps {
  isOpen: boolean;
  onClose: () => void;
  casting: Casting | null;
  onEdit: (casting: Casting) => void;
  onDelete: (castingNo: number) => void;
}

export default function CastingViewModal({ 
  isOpen, 
  onClose, 
  casting, 
  onEdit, 
  onDelete 
}: CastingViewModalProps) {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Agency #${casting.casting_company_no}`}
      size="lg"
    >
      <div className="p-6.5">
        <div className="mb-6 grid grid-cols-1 gap-6 md:grid-cols-2">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">{casting.company_no}</p>
          </div>

          <div>
            <h6 className="mb-2 text-sm font-medium text-black dark:text-white">Address No</h6>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {casting.address_no === 0 ? "No address assigned" : casting.address_no}
            </p>
          </div>
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {casting.contact1 || "Not specified"}
            </p>
          </div>
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {casting.contact2 || "Not specified"}
            </p>
          </div>
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400">
            {casting.submission_preference || "Not specified"}
          </p>
        </div>

        <div className="mb-6 grid grid-cols-1 gap-6 md:grid-cols-2">
          <div>
            <h6 className="mb-2 text-sm font-medium text-black dark:text-white">Seeking</h6>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {casting.seeking || "Not specified"}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-2 md:grid-cols-3">
            <div className="flex items-center">
              <span className={`inline-flex rounded-full bg-opacity-10 py-1 px-3 text-xs font-medium ${
                casting.talk_variey ? "bg-success text-success" : "bg-warning text-warning"
              }`}>
                Talk Variety: {casting.talk_variey ? "Yes" : "No"}
              </span>
            </div>
            <div className="flex items-center">
              <span className={`inline-flex rounded-full bg-opacity-10 py-1 px-3 text-xs font-medium ${
                casting.bi_coastal ? "bg-success text-success" : "bg-warning text-warning"
              }`}>
                Bi-Coastal: {casting.bi_coastal ? "Yes" : "No"}
              </span>
            </div>
            <div className="flex items-center">
              <span className={`inline-flex rounded-full bg-opacity-10 py-1 px-3 text-xs font-medium ${
                casting.primetime ? "bg-success text-success" : "bg-warning text-warning"
              }`}>
                Primetime: {casting.primetime ? "Yes" : "No"}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6">
          ...
          <span className={`inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium ${
            casting.archived ? "bg-danger text-danger" : "bg-success text-success"
          }`}>
            {casting.archived ? "Archived" : "Active"}
          </span>
        </div>
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={() => onDelete(casting.casting_company_no)}
