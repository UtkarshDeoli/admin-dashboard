...
interface CastingListProps {
  castings: (Casting & { company_name?: string })[];
  onEdit: (agency: Casting) => void;
  onDelete: (agencyNo: number) => void;
  onView: (agency: Casting) => void;
  onComments: (agency: Casting) => void;
  deleting: number[];
}
      ...
      <table className="w-full table-auto">
        ...
        <tbody>
          {castings.map((casting) => {
            const isDeleting = deleting.includes(casting.casting_company_no);

            return (
              <tr key={casting.casting_company_no} className="border-b border-stroke dark:border-strokedark">
                <td className="py-5 px-4">
                  <p className="text-black dark:text-white">{casting.casting_company_no}</p>
                </td>
                <td className="py-5 px-4">
                  <p className="text-black dark:text-white font-medium">
                    {casting.company_name || `Company #${casting.company_no}`}
                  </p>
                </td>
                <td className="py-5 px-4">
                ...
                <td className="py-5 px-4">
                  <p className="text-sm text-black dark:text-white">
                    {casting.seeking ? (
                      casting.seeking.length > 50
                        ? `${casting.seeking.substring(0, 50)}...`
                        : casting.seeking
                    ) : "-"}
                  </p>
                </td>
                <td className="py-5 px-4">
                  <span className={`inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium ${
                    casting.submission_preference
                      ? "bg-success text-success"
                      : "bg-warning text-warning"
                  }`}>
                    {casting.submission_preference ? "Yes" : "No"}
                  </span>
                </td>
                <td className="py-5 px-4">
                  <div className="flex items-center space-x-3.5">
                    ...
                    <button
                      ...
                      onClick={() => !isDeleting && onDelete(casting.casting_company_no)}
                      title="Delete"
                      disabled={isDeleting}
                    >
                      {isDeleting ? (
