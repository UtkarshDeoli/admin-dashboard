...
interface AgencyListProps {
  agencies: (Agency & { company_name?: string })[];
  onEdit: (agency: Agency) => void;
  onDelete: (agencyNo: number) => void;
  onView: (agency: Agency) => void;
  onComments: (agency: Agency) => void;
  deleting: number[];
}
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full table-auto">
        ...
        <tbody>
          {agencies.map((agency) => {
            const isDeleting = deleting.includes(agency.agency_no);
            
            return (
              <tr key={agency.agency_no} className="border-b border-stroke dark:border-strokedark">
                <td className="py-5 px-4">
                  <p className="text-black dark:text-white">{agency.agency_no}</p>
                </td>
                <td className="py-5 px-4">
                  <p className="text-black dark:text-white font-medium">
                    {agency.company_name || `Company #${agency.company_no}`}
                  </p>
                </td>
                <td className="py-5 px-4">
                ...
                <td className="py-5 px-4">
                  <p className="text-sm text-black dark:text-white">
                    {agency.represents ? (
                      agency.represents.length > 50 
                        ? `${agency.represents.substring(0, 50)}...`
                        : agency.represents
                    ) : "-"}
                  </p>
                </td>
                ...
                <td className="py-5 px-4">
                  <div className="flex items-center space-x-3.5">
                    ...
                    <button
                      ...
                      onClick={() => !isDeleting && onDelete(agency.agency_no)}
                      title="Delete"
                      disabled={isDeleting}
                    >
                      {isDeleting ? (
